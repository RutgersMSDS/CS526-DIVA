...
function firstPyramidCall()
{
    ...
    addHtmlElements();
    pyramidSlider();
    drawPyramidChart('United States');
    getChoroplethData()
}
...
function addHtmlElements()
{
    ...
    gPlayPause.append("rect")
        .attr('id','playPauseButton')
        .attr('fill','lawngreen')
        .attr('width',80)
        .attr('height',30)
        .on('mousedown.log',function(d){
            //currentText = d3.select('#PlayPause').text()
        ...
        .on('mouseout',function(d){
                store.play = false
                pausePyramid();
                d3.select('#playPauseButton').attr('fill','lawngreen')
                d3.select('#TextPlayPause').text('Play').attr('fill','red')
        })

    gPlayPause.append('text')
                .text('Play')
                .attr('fill','red')
                .attr('id','TextPlayPause')
...
/*function playPyramid()
{
        ...
        store.slider.value(store.currentYear+1)
        now = Date.now()
}

function pausePyramid()
{
...
function pyramidSlider()
{
    ...
    slider = d3
        ...
        .on('onchange', val => {
            ...
            //removePyramids();
            drawPyramidChart(store.countryName);
        });
    store.slider = slider;
    gSlider.call(slider); 
}
